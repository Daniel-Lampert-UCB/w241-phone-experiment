...
data[, iphone_binary := ifelse(iphone == 'IPhone', 1, 0)]
data[is.na(iphone_binary), iphone_binary:=0]
```


```{r}
simple_itt <- data[, lm(screen_time_avg_final ~ treatment + screen_time_avg_initial + as.factor(country) + iphone_binary)]
...
```
# Use Ivyreg to calculate CACE with SE
```{r}
library("ivreg")
ivyreg <- data[, ivreg(screen_time_avg_final ~ treatment |greyscale_binary + screen_time_avg_initial + as.factor(country) + iphone_binary)]
ivyreg$vcovCL_val <- vcovCL(ivyreg)
